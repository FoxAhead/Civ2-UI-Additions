...
begin
  ...
  if Key = 'PurgeMessagesIntervalMs' then
    Result := Options.PurgeMessagesIntervalMs;
  if Key = 'MessageWaitTimeoutMinMs' then
    Result := Options.MessageWaitTimeoutMinMs;
  if Key = 'MessageWaitTimeoutMaxMs' then
    Result := Options.MessageWaitTimeoutMaxMs;
  if Key = 'MessageWaitTimeoutThresholdMs' then
    Result := Options.MessageWaitTimeoutThresholdMs;
...
begin
  ...
  if Key = 'PurgeMessagesIntervalMs' then
    Options.PurgeMessagesIntervalMs := Value;
  if Key = 'MessageWaitTimeoutMinMs' then
    Options.MessageWaitTimeoutMinMs := Value;
  if Key = 'MessageWaitTimeoutMaxMs' then
    Options.MessageWaitTimeoutMaxMs := Value;
  if Key = 'MessageWaitTimeoutThresholdMs' then
    Options.MessageWaitTimeoutThresholdMs := Value;
  if Key = 'SocketBufferOn' then
